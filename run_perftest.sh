...
apt-get update
apt-get install -y git jq telegraf
...
if [ "${CIRCLE_TEARDOWN}" = "true" ]; then
  curl --request POST \
    --url https://circleci.com/api/v2/project/github/influxdata/plutonium/terraform_destroy_callback \
    --header 'Circle-Token: ${CIRCLE_TOKEN}' \
    --header 'content-type: application/json' \
    --data '{"parameters":{"tf_teardown": true, "tf_teardown_branch":"${INFLUXDB_VERSION}", "tf_teardown_build_num":"${CIRCLE_BUILD_NUM}", "tf_teardown_datestring":"${CIRCLE_TEARDOWN_DATESTRING}"}}'
fi
